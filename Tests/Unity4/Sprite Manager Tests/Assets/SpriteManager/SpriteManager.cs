        ...
        for (int row = 0; row < rows; ++row)
        {
            for (int col = 0; col < cols && cellCount < totalCells; ++col)
            {
				if(firstAnimRow){
					...
					if(start.x + cellSize.x * ((float)col+1) >= 1) {
						col = cols;
					}					
				} else {
					frames[cellCount].x = cellSize.x * ((float)col);					
	                frames[cellCount].y = start.y - cellSize.y * ((float)row); // there is a probem HERE
				}
